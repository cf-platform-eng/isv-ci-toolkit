    ...
    cat > "$BATS_TMPDIR/input/environment.json" <<'EOF'
{
    "paver_paving_output": {
        ...
        "optional_ops_manager_dns": {
            "value": "pcf-optional.famousdoctor.cfplatformeng.com"
        }
    }
}
EOF
...
@test "sets target to pcf-optional if UPGRADE=true" {
    UPGRADE=true . ./setup_om.sh "$BATS_TMPDIR/input/environment.json" "$BATS_TMPDIR/input/credentials.json"
    [ "${OM_TARGET}" = "pcf-optional.famousdoctor.cfplatformeng.com" ]
    [ "${OM_USERNAME}" = "pete" ]
    [ "${OM_PASSWORD}" = "super-secret-1" ]
    [ "${OM_SKIP_SSL_VALIDATION}" = "true" ]
}
